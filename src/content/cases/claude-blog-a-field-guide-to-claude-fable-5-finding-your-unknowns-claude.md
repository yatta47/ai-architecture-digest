...
プロンプトは具体的すぎても曖昧すぎても失敗するため、まず自分の立ち位置・経験・前提をClaudeに伝えたうえで協働を始めるのが要点。『ブラインドスポットパス』で未知の未知を言語化させ、ブレストやプロトタイプで『見れば分かるが書き出せない』暗黙の要件を早期に確定し、後工程での高コストな手戻りを避ける。仕様の探索を実装前に前倒しする設計思想。

## 使いどころ

不慣れな領域やコードベースで作業する人、あるいはデザインのように良し悪しを見て初めて判断できる曖昧なタスクで、Claudeを思考パートナーにして未知を減らしたい場面に効く。
