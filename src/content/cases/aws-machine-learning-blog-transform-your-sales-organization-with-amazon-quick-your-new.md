...
繰り返す業務は『スキル』（複数ソースからデータを収集して成果物を生成する再利用可能な自動ワークフロー）として定義し、名前で呼び出す/条件成立時に自動実行する形で全アカウントへ横展開する設計にしている。回答や成果物はQuickSightのダッシュボードやQuick Space（集約ナレッジリポジトリ）といった接続済みの最新データに根拠を置き、メール送信・デッキ生成の前に必ずセラーがレビューして承認するhuman-in-the-loopを挟むことで、鮮度と信頼性を担保している。

## 使いどころ

手作業のCRM更新・見込み客調査・商談準備に時間を奪われ、大量のリードから優先順位を付けきれない営業チームや担当者に効く。既存のSalesforce/HubSpot/Microsoft 365環境にアシスタントを重ねて『売る時間』を取り戻したい組織に向く。
