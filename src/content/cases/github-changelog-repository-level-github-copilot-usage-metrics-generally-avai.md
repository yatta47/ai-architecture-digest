...
従来は組織・ユーザー単位で止まっていた利用計測を、リポジトリ×日付の粒度に落とし込むことで、どのリポジトリでAIが実際にPR活動を駆動しているかを可視化する設計。閲覧はView Copilot Metrics権限に紐づけ、利用メトリクスポリシーの有効化を前提とすることで、粒度を上げつつアクセス制御を担保している。

## 使いどころ

AI導入の効果測定やAI-readinessレポートを整備したい組織で、効果の高いリポジトリに的を絞ってCopilot活用の展開を進めたい場面に効く。
