...
同期APIと非同期処理をDynamoDB Streamsで疎結合にし、POSTでジョブ登録・GETで結果取得というポーリング型インターフェースにすることで29秒制限を回避している。処理失敗時はイベントソースマッピングからSNSトピック経由でエラーハンドリングLambdaへ、そこでも失敗すればEventBridgeアーカイブへ退避してリプレイ可能にする多段のエラー処理を備え、結果の整合性は楽観的ロックで担保する。

## 使いどころ

29秒を超える長時間ジョブや重い計算をREST API経由で受け付けたいが、クライアントを待たせたくないサーバーレス構成に有効。Streams読み取りは2つまで、Lambda実行は15分までといった制約を許容できる場面に向く。
