...
- セットアップ（APIキー発行・Platform接続）を『バンドルされたコネクタ＋MCPサーバー＋スキル』としてエージェントに持たせ、タスクに応じて自動的に呼び出させる
- Codex専用のPlatformコネクタを外し、可搬なスキルと公開Docs MCPだけを移植することで、Claude CodeやCursorなど別のコーディングエージェントにも同じ開発支援を展開できる
