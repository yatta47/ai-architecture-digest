...
- Entra IDをアイデンティティ プロバイダーとして各AWSアカウントに接続し、アクセス認可を条件付きアクセスとリスクベース評価に集約する
- 最も機密性の高いアカウントには、P2ライセンスのPIMによるJust-In-Time昇格・Entra ID Protectionのリスク検知・Defender for Identityの振る舞い監視を段階的に重ね、多層防御(defense-in-depth)を構成する
- 既存環境と並行して構築・テストし、切り替え時期を自分で制御することで移行リスクを抑える

## 使いどころ

- 複数のAWSアカウントを運用し、すでにMicrosoft 365やハイブリッドID基盤でEntra IDを使っている組織
- 追加コストを抑えつつID管理を一元化・強化したい場面
- AWSのIDアーキテクト・管理者・セキュリティアナリストが手引きとして使う場面
