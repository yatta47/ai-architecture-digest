...
- 曖昧な指示（「be helpful」等）を避け、いつ・何を・いつやらないかをトリガー/アクション/例外ルールで記述する
- 推論の深さ(reasoning.effort)は minimal〜xhigh の5段階からタスク複雑度・遅延許容度・失敗コストで選び、本番音声エージェントは low から始める
- プリアンブル（作業中を伝える短い口頭の進捗更新）と書き込み前の確認境界を明示的に制御する
- # Role / # Reasoning / # Preambles などラベル付きの短い節でプロンプトを構造化し、モデルが該当指示を素早く見つけられるようにする
