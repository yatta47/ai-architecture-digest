...
- EDCコネクタを制御プレーン（契約交渉）とデータプレーン（実データ転送）に分離し、責務ごとに独立して扱えるようにする
- SPI（契約定義）・Core（基本実装）・Extensions（AWS等への統合）の3層モジュール構成で拡張性を確保する
- 素の“バニラ”コネクタはAWS拡張を含まないため、version catalog・launcherモジュール・settingsファイルを使ったGradleのプラグイン型ビルドで、S3やSecrets Manager等のマネージドサービスをネイティブ統合したカスタムビルドを組み立てる

## 使いどころ

- クロス組織のデータ共有基盤（データスペース）をAWS上で構築したいアーキテクト
- ポリシー・信頼に基づく相互運用可能なデータ交換を標準準拠で実現したいプラットフォーム担当者
